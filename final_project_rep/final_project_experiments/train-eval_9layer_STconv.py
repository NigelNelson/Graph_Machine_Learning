...
class STGCN(torch.nn.Module):
    def __init__(self, feature_dim, hidden_dim, num_classes):
        ...
        self.stconv4 = st_gcn(hidden_dim, hidden_dim, kernel_size)
        self.stconv5 = st_gcn(hidden_dim, hidden_dim, kernel_size)
        self.stconv6 = st_gcn(hidden_dim, hidden_dim, kernel_size)
        self.stconv7 = st_gcn(hidden_dim, hidden_dim, kernel_size)
        self.stconv8 = st_gcn(hidden_dim, hidden_dim, kernel_size)
    ...
    def forward(self, x, edge_index, batch_index):
        ...
        hidden = self.stconv4(hidden, edge_index)
        hidden = self.stconv5(hidden, edge_index)
        hidden = self.stconv6(hidden, edge_index)
        hidden = self.stconv7(hidden, edge_index)
        hidden = self.stconv8(hidden, edge_index)
...
model = STGCN(feature_dim=feature_dim, hidden_dim=hidden_dim, num_classes=4)
print(model)
...
for train_index, test_index in kf.split(X, y_labels):
    ...
    # model = STGCN(X_train.shape[2], hidden_dim)
